from .node import SVG
from .dsl import TagFactory
from .viz_scale import LinearScale, BandScale
from .viz_mark import line, text, group
...
def axis(scale, orientation: str, tick_count: int = 5,
    ...
    return _Axis(scale, orientation, ticks, fmt, cls)
...
def _build_axis(scale, orientation: str, ticks: list,
                fmt, cls: str):
    ...
    for tick in ticks:
        pos   = scale(tick)
        label = fmt(tick)
        children.append(_tick_group(pos, label, orientation))
...
def _tick_group(pos: float, label: str, orientation: str):
